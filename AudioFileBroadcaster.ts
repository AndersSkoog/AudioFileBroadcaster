...
/*
const tidstangesel_broadcast = {
    "path":path.join(__dirname,"../../assets/tidstangsel.wav"),
    "bit_depth":16,
    "sr":48000,
    "stereo":true,
    "chunk_dur":10
...
//module.exports.EternalAudioFileBroadcaster = EternalAudioFileBroadcaster;
//module.exports.getWavInfo = getWavInfo;




/*
const file_dur             = ((60 * 4) * 60) + (60 * 25) + 55;
const file_path            = "";
const file_sr              = 48000;
const bit_depth            = 16;
const channels             = 2;
const buf_size             = file_sr * (bit_depth / 8) * channels;
let play_cursor            = 0;
let loop                   = true;
let play_buf               = null;
...
async function loadFile() {
    if (!file_data) {
        try {
            let fh = await fsPromises.open(file_path, 'r');
            file_data = await fh.readFile();
            await fh.close();
...
function startStream(){
    if(file_data && state === "stoped"){
      interval_id = startInterval(()=> {
        let offset = (play_cursor % (file_dur - 1)) * buf_size;
        play_buf   = Buffer.copyBytesFrom(file_data,offset,buf_size);
        if(loop){play_cursor += 1}
        else {
            if(play_cursor > file_dur - 2){ // ending one sec early to prevent buffer overflow   
